        ...
        for ix, protein in enumerate(dataset_valid):
            ...
            if args.score_only:
                ...
                for fc in range(1 + loop_c):
                    ...
                    np.savez(structure_sequence_score_file,
                             score=native_score,
                ...
                conditional_probs_only_file = base_folder + '/conditional_probs_only/' + batch_clones[
                    0]['name']
                ...
                unconditional_probs_only_file = base_folder + '/unconditional_probs_only/' + batch_clones[
                    0]['name']
                    ...
                    for temp in temperatures:
                        for j in range(NUM_BATCHES):
                            ...
                            for b_ix in range(BATCH_COPIES):
                                ...
                                seq_recovery_rate = torch.sum(
                                    torch.sum(torch.nn.functional.one_hot(
                                        S[b_ix], 21) * torch.nn.functional.
                                              one_hot(S_sample[b_ix], 21),
                                              axis=-1) *
                                    mask_for_loss[b_ix]) / torch.sum(
                                        mask_for_loss[b_ix])
                                ...
                                if b_ix == 0 and j == 0 and temp == temperatures[
                                        0]:
                                    start = 0
                                    end = 0
                                    list_of_AAs = []
                                ...
                                seq_rec_print = np.format_float_positional(
                                    np.float32(seq_recovery_rate.detach().cpu(
                                    ).numpy()),
                                    unique=False,
                                    precision=4)
                ...
                if args.save_score:
                    np.savez(score_file,
                             ...
                             global_score=np.array(global_score_list,
                                                   np.float32))
                if args.save_probs:
                    ...
                    np.savez(probs_file,
                             ...
                             log_probs=np.array(all_log_probs_concat,
                                                np.float32),
...
if __name__ == "__main__":
    ...
    argparser.add_argument("--suppress_print",
                           type=int,
                           default=0,
    ...
    argparser.add_argument("--path_to_model_weights",
                           type=str,
                           default="",
    ...
    argparser.add_argument(
        ...
        default="v_48_020",
        help=
        "ProteinMPNN model name: v_48_002, v_48_010, v_48_020, v_48_030; v_48_010=version with 48 edges 0.10A noise"
    )
    argparser.add_argument(
        "--use_soluble_model",
        action="store_true",
        default=False,
        help=
        "Flag to load ProteinMPNN weights trained on soluble proteins only.")
    ...
    argparser.add_argument(
        ...
        default=0,
        help=
        "0 for False, 1 for True; save MPNN predicted probabilites per position"
    )
    ...
    argparser.add_argument(
        ...
        default="",
        help=
        "score provided input sequence in a fasta format; e.g. GGGGGG/PPPPS/WWW for chains A, B, C sorted alphabetically and separated by /"
    )
    ...
    argparser.add_argument(
        ...
        default=0,
        help=
        "0 for False, 1 for True; output unconditional probabilities p(s_i given backbone) in one forward pass"
    )
    ...
    argparser.add_argument("--num_seq_per_target",
                           type=int,
                           default=1,
    ...
    argparser.add_argument("--jsonl_path",
                           type=str,
    ...
    argparser.add_argument(
        ...
        default='',
        help=
        "Path to a dictionary specifying which chains need to be designed and which ones are fixed, if not specied all chains will be designed."
    )
    argparser.add_argument("--fixed_positions_jsonl",
                           type=str,
                           default='',
    ...
    argparser.add_argument(
        ...
        default='',
        help=
        "Path to a dictionary which specifies AA composion bias if neededi, e.g. {A: -1.1, F: 0.7} would make A less likely and F more likely."
    )

    argparser.add_argument("--bias_by_res_jsonl",
                           default='',
    ...
    argparser.add_argument(
        ...
        default='',
        help=
        "Path to a dictionary which specifies which amino acids need to be omited from design at specific chain indices"
    )
    argparser.add_argument("--pssm_jsonl",
                           type=str,
                           default='',
                           help="Path to a dictionary with pssm")
    argparser.add_argument(
        "--pssm_multi",
        type=float,
        default=0.0,
        help=
        "A value between [0.0, 1.0], 0.0 means do not use pssm, 1.0 ignore MPNN predictions"
    )
    argparser.add_argument(
        "--pssm_threshold",
        type=float,
    ...
    argparser.add_argument("--pssm_log_odds_flag",
                           type=int,
                           default=0,
                           help="0 for False, 1 for True")
    argparser.add_argument("--pssm_bias_flag",
                           type=int,
                           default=0,
                           help="0 for False, 1 for True")

    argparser.add_argument("--tied_positions_jsonl",
                           type=str,
                           default='',
