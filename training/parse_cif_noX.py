...
ATOM_NAMES = [
    ...
    ("N", "CA", "C", "O", "CB", "CG", "CD1", "CD2", "CE2", "CE3", "NE1", "CZ2",
     "CZ3", "CH2"),  # trp
...
aa2idx = {
    (r, a): i
    for r, atoms in zip(RES_NAMES, ATOM_NAMES)
    ...
    for i in range(L):
        for j, xyz_ij in enumerate(xyz[i]):
            ...
            f.write("%-6s%5s %4s %3s %s%4d    %8.3f%8.3f%8.3f%6.2f%6.2f\n" %
                    ("ATOM", ctr, a, r, "A", i + 1, xyz_ij[0], xyz_ij[1],
                     xyz_ij[2], 1.0, bfac[i, j]))
            if a == 'CA':
                idx.append(i)
            ctr += 1
...
def TMalign(chainA, chainB):
    ...
    # run TMalign
    tm = subprocess.Popen('/home/aivan/prog/TMalign %s %s -m %s' %
...
def parseAssemblies(data, chids):
    ...
    if (assembly_data is None) or (assembly_gen is None) or (oper_list
                                                             is None):
        return xforms
...
def parse_mmcif(filename):
    ...
    pdb2asym = dict({(r[pdbx_poly_seq_scheme.getIndex('pdb_strand_id')],
    ...
    num2seq = {
        r[entity_poly.getIndex('entity_id')]:
        r[entity_poly.getIndex('pdbx_seq_one_letter_code_can')].replace(
            '\n', '')
        for r in entity_poly.getRowList()
        if r[entity_poly.getIndex('type')] == 'polypeptide(L)'
    }
    ...
    if (data.getObj('em_3d_reconstruction') is not None) and (res is None):
        try:
            res = float(
                data.getObj('em_3d_reconstruction').getValue('resolution', 0))
        except:
            res = None
    ...
    metadata = {
        'method':
        data.getObj('exptl').getValue('method', 0).replace(' ', '_'),
        'date':
        data.getObj('pdbx_database_status').getValue(
        ...
        seq,
        'id':
        data.getObj('entry').getValue('id', 0)
    }
